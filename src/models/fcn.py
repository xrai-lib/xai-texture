...
def train_fcn(dataset, data_loader):
    ...
    if torch.cuda.is_available():
        ...
        if num_gpus > 1:  # If multiple GPUs available, use DataParallel for parallel processing
...
def test_fcn(result_path, choice, data_loader):
    ...
    add_to_test_results(result_path, choice, average_iou)  # Add results to test results file
