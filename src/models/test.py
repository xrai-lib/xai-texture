...
def create_data_loader(dataset_choice):
    ...
    if dataset_choice == 10:
        images_dir = config.patch_dataset_path + '/test/images'
    else:
        images_dir = config.TEM_dataset_path + '/test/textures/Feature_' + str(dataset_choice)

    masks_dir = config.patch_dataset_path + '/test/masks'
...
def test_model():
    ...
    if model_choice == 1:
         ...
         test_unet(config.results_path + '/UNET_test.csv', dataset_choice,  test_images_dir, mask_images_dir)
